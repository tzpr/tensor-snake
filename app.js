async function app() {
  ...
  const buttons = document.getElementsByClassName("training-button");
  const predictButton = document.getElementsByClassName(" predict")[0];
  const classes = ["up", "down", "left", "right", "middle"];
  ...
  async function runPredictions() {
      while (true) {
          if (classifier.getNumClasses() > 0) {
              ...
              predictionParagraph.innerText = `prediction: ${classes[result.label]},
              probability: ${result.confidences[result.label]}`;
              console.log('result:', classes[result.label]);
              control(classes[result.label]);
              img.dispose(); 
          }
          await tf.nextFrame(); 
      }
